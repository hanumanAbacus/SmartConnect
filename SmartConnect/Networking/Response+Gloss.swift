...
public extension Response {
  ...
  public func mapObject<T: Gloss.JSONDecodable>(_ type: T.Type) throws -> T {
    guard
  ...
  /// Throws a JSONMapping error on failure.
  public func mapObject<T: Gloss.JSONDecodable>(_ type: T.Type, forKeyPath keyPath: String) throws -> T {
    guard
  ...
  /// Throws a JSONMapping error on failure.
  public func mapArray<T: Gloss.JSONDecodable>(_ type: T.Type) throws -> [T] {
    guard
      let json = try mapJSON() as? [JSON]
    else {
      throw MoyaError.jsonMapping(self)
  ...
  /// Throws a JSONMapping error on failure.
  public func mapArray<T: Gloss.JSONDecodable>(_ type: T.Type, forKeyPath keyPath: String) throws -> [T] {
    guard
      let json = try mapJSON() as? NSDictionary,
      let nested = json.value(forKeyPath: keyPath) as? [JSON]
    else {
